function [feature_table] = extractFeatures(dataChTimeTr, includedFeatures, Fs)
    % extractFeatures - Extract specified features from input data.
    %   [feature_table] = extractFeatures(dataChTimeTr, includedFeatures, Fs)
    %   extracts features specified in includedFeatures from the input
    %   dataChTimeTr and returns a feature table.
    %
    %   Input:
    %   - dataChTimeTr: Input data matrix with multiple channels (columns)
    %   - includedFeatures: Cell array specifying the features to extract
    %   - Fs: Sampling frequency of the data
    %
    %   Output:
    %   - feature_table: Table containing the extracted features.
    ...
    for f = 1:length(includedFeatures)
        ...
        %% Put feature values (fvalues) into a table with appropriate name
